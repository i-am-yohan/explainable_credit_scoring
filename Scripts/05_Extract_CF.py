...
if __name__ == '__main__':
    ...
    def Find_CF(Search_ID, CF_Score, Feat_Chck=[]):  # , Tolerance):
        ...
        ##Score_Dim = (Expl_DF[['score']] >= CF_Score - Tolerance) & (Expl_DF[['score']] <= CF_Score + Tolerance)
        Score_Dim = Expl_DF[['score']] > CF_Score
    ...
    sum_score = lambda x: sum(score_df['score'][score_df['feature'].isin(x)]) / len(x)
    ...
    def Explain(Search_ID, CF_Score, N_samples):
        ...
        Bool_Vec = np.array([0 for i in range(len(Features))])

        pre_0_df = {}
        for feat in Features:
            pre_0_df[feat] = False

        def CF_Score_Calc(Search_ID, CF_Score, feat_i):
            ...
            CF_Test_DF = Find_CF(Search_ID, CF_Score, feat_i)
        ...
        def Out():
            ...
            Explain_out = {}
            Gentic_df_out = Gentic_df.loc[Gentic_df['Score'] >= CF_Score, :]
            ...
            for i in range(Gentic_df_out.shape[0]):
                ...
                CF_Iter = Find_CF(Search_ID, CF_Score, Feat_vec)
        ...
        while 1 < 2:
            ...
            Gentic_df_i_gen = Gentic_df.loc[Gentic_df['Score'] >= min(
                Gentic_df[['Score']].sort_values('Score', ascending=False).head(N_samples)['Score']), :].copy()
            ...
            for feats in Cross_Breed_Word:
                Gentic_df_i = CF_Score_Calc(Search_ID, CF_Score, feats)
    ...
    CF = Explain(args.acc_id, args.target_score, args.n_samples) #This is an adjustable parameter, in fact all of them are!!

    # Counterfactual find table
    CF_Df_val = pd.DataFrame(CF['Counterfactual_Table'].drop(['sk_id_curr', 'target', 'pd', 'predicted_default', 'score'],
                                                             axis=1).stack()).copy()
    ...
    CF_Df_val = CF_Df_val.drop('Case').reset_index().rename(columns={'level_0': 'CF_ID', 'level_1': 'feature'}).copy()

    CF_Df_bool = pd.DataFrame(
        CF['Binary_Feature_Table'].drop(['Generation', 'Feature_id', 'Score', 'sk_id_curr'], axis=1).stack()).copy()
    ...
    plot_table['CF_Relative_Change'] = plot_table['Counterfactual_Value'] / plot_table['Case_Value']
    # plot_table['CF_Relative_Change'] = plot_table['CF_Relative_Change'].fillna(0)
    plot_table.loc[plot_table['CF_Inclusion_Ind'] == False, 'CF_Relative_Change'] = None
    plot_table['CF_Absolute_Change'] = plot_table['Counterfactual_Value'] - plot_table['Case_Value']
    # plot_table['CF_Absolute_Change'] = plot_table['CF_Absolute_Change'].fillna(0)
    plot_table.loc[plot_table['CF_Inclusion_Ind'] == False, 'CF_Absolute_Change'] = None

    # Score Table
    CF_tbl_raw = CF['Counterfactual_Table'][['sk_id_curr', 'score', 'predicted_default']].rename(
        columns={'score': 'raw_score'})
    CF_tbl_tar = CF['Binary_Feature_Table'][['Score']].rename(columns={'Score': 'target_score'})
    ...
    #push to DW
    plot_table.to_sql('plot_table', engine, schema='expl', if_exists='replace')
