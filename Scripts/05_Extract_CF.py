...
if __name__ == '__main__':
    ...
    def Explain(Search_ID, Target_Score, Lower_Bound, Upper_Bound, N_samples):
        ...
        def CF_Score_Calc(Search_ID, Lower_Bound_i, Upper_Bound_i, feat_i):
            ...
            Gentic_df_i = pd.DataFrame(pre_0_df, index=[0]) #Create iteration of Gentic_df that will be appended to the main table
            Gentic_df_i['Generation'] = 1
            Gentic_df_i[feat_i] = True #Change the features to exclude in NUN in Gentic DF to true. Used to identify the features 
            CF_Test_DF = Find_CF(Search_ID, Lower_Bound_i, Upper_Bound_i, feat_i) #Deploy the NUN algo
            CF_Feat_vec = X_Full.loc[[CF_Test_DF.loc['Counterfactual', 'sk_id_curr']]].copy() #Extract the row from X_Full for the counterfactual
            Test_Feat_vec = X_Full.loc[[Search_ID]].copy() #Extract the row from X_Full for the case that requires explanation
            Test_Feat_vec[feat_i] = CF_Feat_vec[feat_i].values  #Important!! Replace the values in the case with those of the counterfactual
            Prdict = Model.predict_proba(X_Full.loc[[Search_ID]])[:, 1]
            New_PD = Model.predict_proba(Test_Feat_vec)[:, 1] #Make a prediction using the case with the new counterfactual values
        ...
        def Find_CF(Search_ID, Lower_Bound_j, Upper_Bound_j, Feat_Chck=[]):  # , Tolerance):
            ...
            :return:
            '''
            Feat_Chck_plus = Feat_Chck + ['sk_id_curr', 'target', 'pd', 'predicted_default', 'score'] #Create a vector of features to exclude. 
            Init_Row = Expl_DF[Expl_DF['sk_id_curr'] == Search_ID] #Extract the case from the Explanation DF
            Init_Row.index = ['Case']
            Init_Row_norm = Expl_DF_norm[Expl_DF_norm['sk_id_curr'] == Search_ID] #Extract the normalized features for the case that requires explanation
            Init_arr_norm = np.array(Init_Row_norm.drop(Feat_Chck_plus, axis=1)) #remove features that are to be excluded from NUN
            Score_Dim = (Expl_DF[['score']] >= Lower_Bound_j) & (Expl_DF[['score']] <= Upper_Bound_j) #Create a boolean vector to subset the DF to only the scores within the required threshold 
            ...
            Distance = np.sum(np.abs(CF_arr_norm - Init_arr_norm), axis=1) #Calculate the normalized uclidian distance between the case and all of the counterfactuals
            CF_df['Distance'] = Distance 
            CF = CF_df[CF_df['Distance'] == min(CF_df['Distance'])].drop('Distance', axis=1) #Find the observations with the smallest distance
        ...
        def Out():
            ...
            #Explain_out = {}
            Gentic_df_out = Gentic_df.loc[(Gentic_df['Score'] >= Lower_Bound) & (Gentic_df['Score'] <= Upper_Bound), :] #Extract cases in main DF that satisfy the criteria
            ...
            for i in range(Gentic_df_out.shape[0]): #Loop through them
                ...
                if i == 0:
                    ...
                    CF_out = CF_out.append(CF_Iter.loc['Counterfactual_{}'.format(i), :]).copy() #Append to output component
        ...
        while 1 < 2:
            ...
            Gentic_df['Score_diff'] = abs(Gentic_df['Score'] - Target_Score) #Find the distance between each NUN and the target score
            ...
            Cross_Breed_Word = list([list(features_np[i]) for i in Cross_Breed]) #Crete a word vector for subsetting purposes
